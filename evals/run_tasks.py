...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        type=str,
        nargs="+",
        default=[f"./evals/datasets/frames_{testset_name}_set.csv"],
...
def answer_single_question(example, model, answers_file, action_type, search_model_id=None):
    if action_type == "vanilla":
        ...
        agent = CodeAgent(
            tools=[OpenSearchTool(base_model)],
            model=model,
        ...
        agent = ToolCallingAgent(
            tools=[OpenSearchTool(base_model), PythonInterpreterTool()],
            model=model,
            additional_authorized_imports=["numpy"],
            max_steps=15,
        )
    ...
    try:
        ...
        if results:
            print(results)
            with open("error_result.json", "w", encoding="utf-8") as f:
                json.dump(results, f)
